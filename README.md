...
# ssm422课程案例库平台的设计与实现+jsp

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=21)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本案例展示平台也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让案例信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，案例展示平台的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，案例展示平台是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此案例展示平台为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
案例展示平台的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
图4.1 程序工作的原理图
## 4.2 系统功能结构设计
在分析并得出使用者对程序的功能要求时，就可以进行程序设计了。如图4.2展示的就是管理员功能结构图，管理员主要负责填充案例和其类别信息，并对已填充的数据进行维护，包括修改与删除，管理员也需要对新闻,用户,任务等进行管理。
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|2|yonghu\_id|Integer|制作人|是|
|3|anli\_name|String|案例名称|是|
|4|anli\_zhuti\_types|Integer|制作主题|是|
|5|anli\_types|Integer|制作方法|是|
|6|anli\_xingshi|String|案例形式|是|
|7|anli\_photo|String|案例缩略图|是|
|8|anli\_file|String|案例文件|是|
|9|anli\_content|String|案例介绍|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|11|anli\_id|Integer|案例|是|
|12|yonghu\_id|Integer|用户|是|
|13|insert\_time|Date|收藏时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|15|dic\_code|String|字段|是|
|16|dic\_name|String|字段名|是|
|17|code\_index|Integer|编码|是|
|18|index\_name|String|编码名字|是|
|19|super\_id|Integer|父字段id|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|21|news\_name|String|新闻名称|是|
|22|news\_types|Integer|新闻类型|是|
|23|news\_photo|String|新闻图片|是|
|24|insert\_time|Date|新闻时间|是|
|25|news\_content|String|新闻详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|27|yonghu\_id|Integer|发布人|是|
|28|renwu\_name|String|任务名称|是|
|29|renwu\_types|Integer|任务类型|是|
|30|renwu\_file|String|附件|是|
|31|insert\_time|Date|发布时间|是|
|32|news\_content|String|任务详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|38|yonghu\_number|String|编号|是|
|39|yonghu\_name|String|姓名|是|
|40|yonghu\_phone|String|手机号|是|
|41|yonghu\_id\_number|String|身份证号|是|
|42|yuangong\_photo|String|照片|是|
|43|xueyuan\_types|Integer|学院|是|
|44|nianji\_types|Integer|年级|是|
|45|role\_types|Integer|权限|是|
...
图5.2 公告管理页面
### 5.1.3 案例管理
如图5.3显示的就是案例管理页面，此页面提供给管理员的功能有：新增案例,修改案例,删除案例。

![](/md/blog.012.png)

图5.3 案例管理页面
### 5.1.4 制作方法管理
如图5.4显示的就是制作方法管理页面，此页面提供给管理员的功能有：新增制作方法,修改制作方法,删除制作方法。
...
## 5.2前台首页功能模块
` `案例展示平台，在案例展示平台可以查看案例信息、我的、新闻和跳转到后台等内容，如图5-17所示。

![](/md/blog.014.png)

图5-17系统首页界面图

登录,通过注册填写账户和密码进行登录，如图5-18所示。

![](/md/blog.015.png)

图5-18登录面图

案例信息，在案例信息页面可以查看案例的详细信息，如图5-19所示。 
